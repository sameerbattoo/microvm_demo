...
⚠️  DEPRECATED: With VM rotation in place, VMs rotate before hitting max
lifetime — AWS never auto-terminates a running VM. This test was a one-time
investigation that proved AWS DOES fire /terminate on auto-termination of
a RUNNING VM. The finding informed the rotation design.

RESULT (ran 2026-07-21):
  ✅ AWS DOES fire /terminate hook when it auto-terminates a RUNNING VM.
  Resuming the VM before expiry is sufficient to trigger the hook and
  save the checkpoint. This confirmed our rotation approach is sound.

To re-run this test, temporarily set MAX_LIFETIME_SECONDS=240 in config.sh
...
TEST FLOW:
  1. Launch VM: maxDuration=240s (from config), idleTimeout=60s, checkpoint=True
  2. Execute code (create state)
  3. Wait for VM to suspend (~60s idle)
  4. At t=210s (30s before max duration), resume the VM
  5. Let AWS terminate it at t=240s (max duration)
...
Usage:
    # Set MAX_LIFETIME_SECONDS=240 in config.sh, then start proxy
    python3 tests/test_resume_before_expire.py
"""

# --- Test code commented out — preserved for reference ---
# To re-enable: uncomment everything below and set MAX_LIFETIME_SECONDS=240
...
# MEMORY_MIB = 2048
# MAX_DURATION_SEC = 240       # 4 minutes — must match MAX_LIFETIME_SECONDS in config.sh
# IDLE_TIMEOUT_SEC = 60
...
if __name__ == "__main__":
    print("⚠️  This test is deprecated (VM rotation makes it obsolete).")
    print("   See docstring for details and how to re-enable if needed.")
